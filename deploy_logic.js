  ...
  try {
    ...
    for (const wf of data.data) {
      if (mapLogic[wf.name]) {
        ...
        // Find the existing webhook node to preserve its ID and path if needed
        ...
        if (existingWebhook && logicWebhook) {
           logicWebhook.id = existingWebhook.id; // Preserve ID
           logicWebhook.parameters.path = existingWebhook.parameters.path; // Preserve path
           
           // Update connections to use the preserved webhook name if it was different
        ...
        wf.nodes = logic.nodes;
        wf.connections = logic.connections;
        
        // Delete Respond to Webhook nodes if logic doesn't use it
        // The webhook node handles response with responseMode "onReceived"
