...
async function check() {
  ...
  try {
    const response = await fetch(`${baseUrl}/executions?workflowId=${wfId}&limit=5`, {
      headers: {
    ...
    console.log('Follow-Up Dispatcher Executions:');
    if (data.data && data.data.length > 0) {
      ...
      console.log('No executions found yet. Waiting for cron trigger...');
    }
  } catch (e) {
    console.error(e);
  }
