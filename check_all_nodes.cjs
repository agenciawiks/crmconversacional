  ...
  try {
    ...
    if (data.data && data.data.resultData && data.data.resultData.runData) {
      ...
      for (const [nodeName, runs] of Object.entries(runData)) {
        runs.forEach((r, idx) => {
          if (r.error) {
            console.log(`Node "${nodeName}" (Run #${idx+1}) error:`, r.error);
          }
          if (r.data && r.data.main && r.data.main.length > 0 && r.data.main[0].length > 0) {
            const out = r.data.main[0][0].json;
            if (nodeName === 'Parse Meta Payload' || nodeName === 'AI Routing Decision' || nodeName === 'If Run AI?') {
              console.log(`Node "${nodeName}" output:`, JSON.stringify(out, null, 2).substring(0, 300));
            }
          }
        });
      }
...
async function check() {
  await getExecution('67658');
  await getExecution('67661');
  await getExecution('67662');
  await getExecution('67663');
  await getExecution('67665');
  await getExecution('67666');
  await getExecution('67667');
}
