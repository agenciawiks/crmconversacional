...
async function run() {
  ...
  const outboundWorkflow = workflows.find(w => w.name.includes("Outbound Send Message") || w.name.includes("Outbound"));
  if (outboundWorkflow) {
    console.log("Found Outbound Workflow!");
    console.log("ID:", outboundWorkflow.id);
    console.log("Name:", outboundWorkflow.name);
    console.log("Active:", outboundWorkflow.active);
  } else {
    console.log("Outbound Workflow not found in list.");
  }
}
