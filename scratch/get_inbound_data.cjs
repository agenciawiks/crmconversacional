const https = require('https');

const apiKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIyN2Y2NjgzMS1iNjE2LTQwZGEtYjZkYS05MGQzZWExMmE0NmIiLCJpc3MiOiJuOG4iLCJhdWQiOiJwdWJsaWMtYXBpIiwianRpIjoiYmNjYzViNWQtOTI4NS00N2I2LWJhOWUtNmZhYjQ1NDM1MTc0IiwiaWF0IjoxNzc5ODE1MjU0fQ.-l3smjKe9_ejhjXd1X7HzdnxROuC2CZQblCC7KJoJYM";

function apiCall(path) {
  return new Promise((resolve, reject) => {
    https.get(`https://n8n-n8n.rh3fr2.easypanel.host/api/v1${path}`, {
      headers: { 'X-N8N-API-KEY': apiKey }
    }, (res) => {
      let data = '';
      res.on('data', chunk => data += chunk);
      res.on('end', () => resolve(JSON.parse(data || '{}')));
    }).on('error', reject);
  });
}

async function run() {
  console.log("Fetching detailed execution data for 38802...");
  const detail = await apiCall(`/executions/38802?includeData=true`);
  const rd = detail.data?.resultData || {};
  if (rd.runData) {
    console.log("=== Node execution run data ===");
    for (const [nodeName, runs] of Object.entries(rd.runData)) {
      console.log(`Node: ${nodeName}`);
      runs.forEach((r, idx) => {
        console.log(`  Run ${idx}: input/output keys:`, Object.keys(r));
        if (r.data?.main?.[0]) {
          console.log(`  Main output data:`, JSON.stringify(r.data.main[0], null, 2));
        }
      });
    }
  } else {
    console.log("No runData found:", detail);
  }
}

run();
