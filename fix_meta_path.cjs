...
async function fix() {
  ...
  // Find GET node and update path
  const getNode = wf.nodes.find(n => n.name === "Meta Verification (GET)");
  if (getNode) {
     getNode.parameters.path = "meta-inbound";
  }
  ...
  console.log("Updated workflow:", result.id);
}
